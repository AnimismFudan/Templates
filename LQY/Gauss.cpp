...
void Gauss()
{
     ...
     for(i=1,p=1;i<=m;p++) //处理第i个方程 消元第p个未知数 
     {
         ...
         if(A[k][p]!=0)
         {
            ...
            for(j=i+1;j<=m;j++)
            {
                ...
                if(B[j]<0) B[j]+=7;
                if(fl)
                   if(B[j]!=0) {printf("Inconsistent data.\n"); return;}
                   else flag=true;
            }
            i++;
         }
     ...
     if(p<n) {printf("Multiple solutions.\n");return;}
     if(A[n][n]==0&&B[n]==0) {printf("Multiple solutions.\n");return;}
     X[n]=Calc(A[n][n],B[n]);
     for(i=n-1;i>=1;i--)
     {
         for(j=i+1,k=0;j<=n;j++)
