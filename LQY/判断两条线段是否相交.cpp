...
struct NodeTp
{
	...
	NodeTp operator-(NodeTp A) {return (NodeTp){(x-A.x),(y-A.y)};}
};

inline int Cross(NodeTp A,NodeTp B) {return A.x*B.y-A.y*B.x;}

bool CheckRec(NodeTp A1,NodeTp A2,NodeTp B1,NodeTp B2) // 快速跨立实验
{
	int x1,x2,x3,x4;
	x1=min(A1.x,A2.x); x2=max(A1.x,A2.x);
	x3=min(B1.x,B2.x); x4=max(B1.x,B2.x);
	if(x2<x3||x4<x1) return false;
	x1=min(A1.y,A2.y); x2=max(A1.y,A2.y);
	x3=min(B1.y,B2.y); x4=max(B1.y,B2.y);
	if(x2<x3||x4<x1) return false;
	return true;
}
